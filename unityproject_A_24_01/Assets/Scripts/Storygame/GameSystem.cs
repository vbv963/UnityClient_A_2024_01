...
public class GameSystem : MonoBehaviour
{
    ...
    public void ApplyChoice(StoryModel.Result result)         //버튼을 누른 결과 연산 함수
    {
        switch (result.resultType)
        {
            ...
            case StoryModel.Result.ResultType.GoToNextStory:        //다음 스토리 진행
                ...
                ChangeState(result);                                //함수를 통해서 스토리 시스템 접근하여 스토리 연출 시작
                break;
    ...
    {
        StoryModel tempStoryModel = FindStoryMode(number);
    ...
    StoryModel FindStoryMode(int number)                    //StoryModel을 되돌려주는 함수 번호로 찾아서 리턴
    {
        StoryModel tempStoryModels = null;
